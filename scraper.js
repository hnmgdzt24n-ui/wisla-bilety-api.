...
async function run() {
  try {
    ...
    console.log("Analizuję tekst za pomocą AI (gemini-1.5-flash)...");
...
UWAGA DOTYCZĄCA BILETÓW (TO SĄ SPRZEDANE BILETY, NIE WOLNE MIEJSCA):
Na stronie liczba SPRZEDANYCH biletów pojawia się jako samotna liczba (np. 15302, 850, 4120) umieszczona w okienku przy banerze meczu.
...
${bodyText.substring(0, 30000)}`;
    ...
    const safeOutput = { 
      updated: new Date().toLocaleString('pl-PL', { timeZone: 'Europe/Warsaw' }) + " (Błąd AI)", 
      events: []
    };
